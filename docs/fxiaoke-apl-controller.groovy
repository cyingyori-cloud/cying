...
class AplCtrlPowerQuote {
    ...
    public HttpResponse callPowerQuote(HttpRequest request) {
        try {
            ...
            def body = Fx.json.toJson(params)
            
            def httpRequest = new HttpRequest(headers, null, body.getBytes("UTF-8"))
            def (Boolean error, HttpResult httpResult, String errorMsg) = Fx.http.post(url, httpRequest)
